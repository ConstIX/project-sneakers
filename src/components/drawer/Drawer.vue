...
<template>
  ...
  <div
    :class="[
      'fixed top-0 z-10 flex h-full w-96 flex-col overflow-auto bg-white px-8 pt-8 transition-all md4:w-full md4:px-5 md4:pt-5',
      drawerVisible ? 'right-0' : '-right-[110%]'
    ]"
  >
    <div class="flex items-center gap-5">
      <button @click="handleCloseDrawer" class="mt-1 rounded-lg border p-1">
    ...
    <div v-else-if="order" class="flex h-full flex-col items-center justify-center">
      <img src="/order.png" alt="..." class="w-20" />
      <div class="mx-auto max-w-[320px] text-center">
        <h3 class="mb-1 text-2xl font-bold text-green-400">Заказ оформлен!</h3>
        <p class="text-gray-400">Ваш заказ #18 скоро будет передан курьерской доставке</p>
      </div>
    </div>
